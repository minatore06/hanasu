...
public class ServerThread extends Thread{
    ...
    private void initialization() throws Exception{
        ...
        nick = inputClient.readLine();//presa del nickname
        System.out.println(nick);
        
        gf.addUtente(this);//aggiunge il thread all'arraylist solo dopo che è venuto a conoscenza del nick
        
        gf.sendAll(nick+" si è connesso alla chat!", this);//mostra a tutti che si è connesso
    ...
    private void comunica()throws Exception{
        ...
        for(;;){
            ...
            if(stringaClient == null){//in caso di chiusura non segnalata, la trasforma in una chiusura segnalata
            ...
            if(stringaClient.indexOf('/')!=-1){
                ...
                switch(cmd){
                    case "dm":
                        if(stringaClient.indexOf(' ', 4)!=-1)/*carattere da rivedere, per indicare fine nome(magari fare con spazio)*/{
                            ...
                            //errore, non ha messo nessun nome(non più valido)
                ...
                if(stringaClient.equals("Logout")){//dovrebbe funzionare
                    ...
                    outputClient.close();
                    client.close();
                    gf.sendAll(nick+" disconnesso!", this);//notifica a tutti gli utenti connessi la disconnessione
